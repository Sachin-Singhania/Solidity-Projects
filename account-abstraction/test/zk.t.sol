...
contract ZkMinimalAccountTest is Test, ZkSyncChainChecker {
    ...
    function setUp() public {
      ...
      function testNonOwnerCannotExecute () public {
        ...
        Transaction memory txn = _createUnsignedTransaction (minimalAccount.owner() ,113, dest, value, functionData);
            //act
        vm.prank(randomuser);
        vm.expectRevert(ZkMinimalAccount.ZK__OnlybootloaderOrOwnerCanCall.selector);
        minimalAccount.executeTransaction(EMPTY_BYTES32,EMPTY_BYTES32,txn); 
      }
    function testZKvalidateTranx() public {
        // Arrange
        address dest = address(usdc);
        uint256 value = 0;
        bytes memory functionData = abi.encodeWithSelector(ERC20Mock.mint.selector, address(minimalAccount), AMOUNT);
    ...
    function _signTransaction(Transaction memory transaction) internal view returns (Transaction memory) {
        bytes32 txnhash= MemoryTransactionHelper.encodeHash(transaction);
        uint8 v;
        bytes32 r;
        bytes32 s;
        uint256 ANVIL_DEFAULT_KEY = 0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80;
        (v, r, s) = vm.sign(ANVIL_DEFAULT_KEY, txnhash);
